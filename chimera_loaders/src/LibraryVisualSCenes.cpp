...
LibraryVisualScenes::LibraryVisualScenes(tinyxml2::XMLElement* _root,
...
void LibraryVisualScenes::target() {
    ...
    tinyxml2::XMLElement* l_nScene = root->FirstChildElement("library_visual_scenes")
        ...
        if (url.compare(l_id) == 0) {
            ...
            for (l_nNode; l_nNode; l_nNode = l_nNode->NextSiblingElement()) {
                ...
                if (l_nDadoNode != nullptr) {
                    carregaNode((Chimera::Node*)pRootNode, l_nDadoNode, l_idR, l_name,
                                l_type);
                }
            }
            return; // pRootNode;
    ...
    throw Chimera::ExceptionChimera(Chimera::ExceptionCode::READ,
                                    "Visual scenes nao encontrado: " + url);
}
...
glm::mat4 LibraryVisualScenes::getTransformation(tinyxml2::XMLElement* _nNode) {
    ...
    if (strcmp(l_tipoTransform, (const char*)"transform") == 0) {
        ...
        throw Chimera::ExceptionChimera(Chimera::ExceptionCode::READ,
                                        "Matrix de transformacao invalida");
...
void LibraryVisualScenes::carregaNode(Chimera::Node* _pNodePai,
                                      tinyxml2::XMLElement* _nNode, const char* _id,
    ...
    for (_nNode; _nNode; _nNode = _nNode->NextSiblingElement()) {
        ...
        if (strcmp(l_nomeElemento, (const char*)"matrix") == 0) {
            ...
            if (pLastNodeDone != nullptr) {
                carregaNode(pLastNodeDone, _nNode->FirstChildElement(), l_id, l_name,
                            l_type);
            } else {
                throw Chimera::ExceptionChimera(Chimera::ExceptionCode::READ,
                                                "Falha, objeto hierarquia: " +
                                                    std::string(l_id));
            }

        } else {
            throw Chimera::ExceptionChimera(Chimera::ExceptionCode::READ,
                                            "Falha, objeto desconhecido: " +
                                                std::string(l_nomeElemento));
        }
    }
}
