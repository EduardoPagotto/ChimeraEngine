...
Solid::Solid(Node* _parent, std::string _name, PhysicsControl* _pWorld)
    : Coord(_parent, EntityKind::SOLID, _name) {
...
void Solid::initTransform(const btTransform& _tTrans, void* pObj) {
    ...
    btRigidBody::btRigidBodyConstructionInfo rBodyInfo(mass, pMotionState,
                                                       pShapeCollision, localInertia);
...
void Solid::applyTorc(const glm::vec3& _torque) {
    ...
    pRigidBody->applyTorque(pRigidBody->getInvInertiaTensorWorld().inverse() *
                            (pRigidBody->getWorldTransform().getBasis() *
                             btVector3(_torque.x, _torque.y, _torque.z)));
