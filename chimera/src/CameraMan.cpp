...
void CameraMan::updateMove() {
    ...
    GLfloat x = glm::dot(along, position ); //  along * (*m_pPosition); //Dot Product (produto escalar) // DotProduct(along, Position); //GLfloat x = DotProduct(along, Position);
...
//     if ( type == CameraType::Land )
//         position -= glm::vec3 ( forward.x() * ! ( _wall[0] && forward.x() * _delta > 0.0 || _wall[1] && forward.x() * _delta < 0.0 ),
//                                 0.0,
...
//     if ( type == CameraType::Land )
//         position -= glm::vec3 ( along.x() * ! ( _wall[0] && along.x() * _delta > 0.0 || _wall[1] && along.x() * _delta < 0.0 ),
//                                 0.0,
