...
HmdDevice::HmdDevice(int _width, int _height, std::string _nome)
    : Video(_nome, KIND_DEVICE::SCREEN, _width, _height) {

    // FIXME: necessario verificar se nao é outyro valor como o do olho +- 960x540
    // (1920x1080 / 2 ou 900x900)
...
void HmdDevice::endDraw() {
    ...
    glReadPixels(0, 0, winSizeW, winSizeH, GL_RGBA8, GL_UNSIGNED_BYTE,
                 &data[0]); // GL_RGBA ??
...
glm::mat4 HmdDevice::getPerspectiveProjectionMatrix(const float& _fov, const float& _near,
...
// void HmdDevice::perspectiveGL( GLdouble fovY, GLdouble aspect, GLdouble zNear, GLdouble
// zFar )//TODO subistituir o executeViewPerspective
//{
