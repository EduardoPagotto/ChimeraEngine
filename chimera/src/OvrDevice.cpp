...
void OvrDevice::initDevice() {
    ...
    if ((hmd = ovrHmd_Create(0)) == nullptr) {
        ...
        if ((hmd = ovrHmd_CreateDebug(ovrHmd_DK2)) != nullptr) {
            ...
            throw ExceptionSDL(ExceptionCode::CREATE,
                               std::string("Falha Criar virtual debug HMD"));
        }
    }
     ...
     * timewrap, which shifts the image before drawing to counter any lattency between the
     * call to ovrHmd_GetEyePose and ovrHmd_EndFrame.
     */
    distort_caps = ovrDistortionCap_Chromatic | ovrDistortionCap_TimeWarp |
                   ovrDistortionCap_Overdrive;
    if (!ovrHmd_ConfigureRendering(hmd, &glcfg.Config, distort_caps, hmd->DefaultEyeFov,
                                   eye_rdesc)) {
...
void OvrDevice::update_rtarg(int width, int height) {
    ...
    glBindTexture(GL_TEXTURE_2D, fb_tex);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, fbTexSize.w, fbTexSize.h, 0, GL_RGBA,
                 GL_UNSIGNED_BYTE, 0);
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, fb_tex,
                           0);
    ...
    if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE) {
        throw ExceptionSDL(ExceptionCode::CREATE,
                           std::string("Falha:Frame buffer incompleto"));
    }

    glBindFramebuffer(GL_FRAMEBUFFER, 0);
    printf("created render target: %dx%d (texture size: %dx%d)\n", width, height,
           fbTexSize.w, fbTexSize.h);
}
...
void OvrDevice::toggleFullScreen(void) {
    ...
    if (fullscreenStatus == false) {
...
#ifdef OVR_OS_LINUX
        /* on linux for now we have to deal with screen rotation during rendering. The
         * docs are promoting not rotating the DK2 screen globally
         */
        ...
        ovrHmd_ConfigureRendering(hmd, &glcfg.Config, distort_caps, hmd->DefaultEyeFov,
                                  eye_rdesc);
#endif
...
void OvrDevice::endDraw() {
     ...
     * properly compensated for lens distortion and chromatic abberation onto the HMD
     * screen.
     */
    glBindFramebuffer(GL_FRAMEBUFFER, 0);
...
void OvrDevice::executeViewOrto(int eyeIndex) {
    ...
    glOrtho(fb_ovr_tex[eyeIndex].OGL.Header.RenderViewport.Pos.x,
            ...
            fb_ovr_tex[eyeIndex].OGL.Header.RenderViewport.Size.h, -1, 1);
...
// 		DrawTextBox(pRender, 0.0f, -textHeight, textHeight, loadMessage.ToCStr(),
// DrawText_HCenter); 		LoadingState = LoadingState_DoLoad;
// 	}
//
...
// Vector2f(EyeRenderDesc[0].PixelsPerTanAngleAtCenter); 	Vector2f orthoScale1   =
...
// ovrMatrix4f_Projection(RenderState.EyeRenderDesc[0].Fov, 0.01f, 10000.f, true);
...
void OvrDevice::executeViewPerspective(const float& _fov, const float& _near,
    ...
    const Vector2f orthoScale =
        Vector2f(1.f) / Vector2f(eye_rdesc[eye].PixelsPerTanAngleAtCenter);

    // perspectiveProjection = ovrMatrix4f_Projection(hmd->DefaultEyeFov[eye], 0.5, 500.0,
    // 1);
    perspectiveProjection = ovrMatrix4f_Projection(eye_rdesc[eye].Fov, 0.1, 1000.0, true);

    orthoProjection[_eye] =
        ovrMatrix4f_OrthoSubProjection(perspectiveProjection, orthoScale, orthoDistance,
     ...
     * we need to construct a view matrix by combining all the information provided by the
     * oculus SDK, about the position and orientation of the user's head in the world.
     */
