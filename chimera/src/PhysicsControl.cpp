...
PhysicsControl::PhysicsControl() {
    ...
    discretDynamicsWorld->setInternalTickCallback(PhysicsControl::doTickCallBack,
        ...
        for (int j = 0; j < numContacts; j++) {
            ...
            if (pt.getDistance() < 0.0f) {
                ...
                if (new_contacts.find(objB) == new_contacts.end()) {
                    ...
                    Node* l_pNodeB =
                        (Node*)objB->getUserPointer(); // rigidbody contem o dado
                    Node* l_pNodeA =
                        (Node*)objA->getUserPointer(); // rigidbody contem o dado

                    if (l_pNodeB) {
                        if (checkAllowCollision(l_pNodeB) == true) {
                            new_contacts[objB] =
                                std::pair<Node*, Node*>(static_cast<Node*>(l_pNodeA),
                                                        static_cast<Node*>(l_pNodeB));
                        }
                ...
                if (new_contacts.find(objA) == new_contacts.end()) {
                    ...
                    if (l_pNodeA) {
                        if (checkAllowCollision(l_pNodeA) == true) {
                            new_contacts[objA] =
                                std::pair<Node*, Node*>(static_cast<Node*>(l_pNodeB),
                                                        static_cast<Node*>(l_pNodeA));
                        }
        ...
        for (it = new_contacts.begin(); it != new_contacts.end(); it++) {
            if (contactActives.find((*it).first) == contactActives.end()) {
                ...
                if (checkAllowCollision((*it).second.first) == true) {
                    sendMessage(KindOp::START_COLLIDE, (*it).second.first,
                                (*it).second.second);
                }
    ...
    if (!contactActives.empty()) {
        for (it = contactActives.begin(); it != contactActives.end(); it++) {
            if (new_contacts.find((*it).first) == new_contacts.end()) {
                ...
                if (checkAllowCollision((*it).second.first) == true) {
                    sendMessage(KindOp::OFF_COLLIDE, (*it).second.first,
                                (*it).second.second);
                }
            }
        }
