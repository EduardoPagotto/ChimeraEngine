...
void conversorVBO(std::vector<unsigned int>& vertexIndex,
    ...
    for (unsigned int face = 0; face < l_numFaces; face++) {
        ...
        for (unsigned int point = 0; point < 3; point++) {
            l_index = fa + point;
            outData.push_back({
                vertexList[vertexIndex[l_index]], normalList[normalIndex[l_index]],
                (textureIndex.size() > 0) ? textureList[textureIndex[l_index]]
                                          : glm::vec2(0.0, 0.0) //,
...
bool getSimilarVertexIndex(VertexData& in_vertex, std::vector<VertexData>& out_vertex,
                           unsigned int& result) {
