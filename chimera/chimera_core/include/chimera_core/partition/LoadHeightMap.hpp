...
namespace ce {
    ...
    class LoadHeightMap {
      ...
      public:
        ...
        void split(TrisIndex& vertexIndexIn, std::vector<TrisIndex>& vTrisIndexOut) const;

      private:
        uint32_t getIndex(const uint32_t& _x, const uint32_t& _z) { return (pImage->w * _z) + _x; }
