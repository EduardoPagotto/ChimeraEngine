...
CameraSpherical::CameraSpherical(std::string _name)
    : Camera(nullptr, CameraType::Spherical, _name) {
...
void CameraSpherical::initTrackBall(void) {
    ...
    vertical = asin((glm::abs(position.z) - glm::abs(direction.z)) /
                    glm::distance(position, direction)) /
               0.017453293f;
    horizontal = asin((glm::abs(position.y) - glm::abs(direction.y)) /
                      glm::distance(position, direction)) /
                 0.017453293f;
}
