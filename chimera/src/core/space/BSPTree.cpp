...
//                 if (alpha.classifyPoly(vertex[th2->idx.s].point, vertex[th2->idx.t].point, vertex[th2->idx.p].point,
//                 &result) !=
//                     SIDE::CP_ONPLANE)
//                     return false;
...
void BspTree::splitTriangle(const glm::vec3& fx, std::shared_ptr<Triangle> _pTriangle, Plane& hyperPlane,
    ...
    _vTriangle.push_front(
        std::make_shared<Triangle>(glm::uvec3(last + 3, last + 4, last + 5), _pTriangle->normal, _pTriangle->splitter));
    ...
    _vTriangle.push_front(
        std::make_shared<Triangle>(glm::uvec3(last + 6, last + 7, last + 8), _pTriangle->normal, _pTriangle->splitter));
