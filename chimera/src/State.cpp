...
void State::apply() {
    ...
    for (map<StateMachine, bool>::iterator iter = this->map_stateMachine.begin();
         iter != map_stateMachine.end(); ++iter)
        setStateMachine(iter->first, iter->second);

    for (map<LightNum, bool>::iterator iter = this->map_light.begin();
         iter != this->map_light.end(); ++iter)
    ...
    for (map<CullFace, bool>::iterator iter = this->map_cullface.begin();
         iter != this->map_cullface.end(); ++iter)
        setEnable(iter->first, iter->second);

    for (map<ClientState, bool>::iterator iter = this->map_clientstate.begin();
         iter != this->map_clientstate.end(); ++iter)
