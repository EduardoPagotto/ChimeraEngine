...
HUD::HUD(Node* _parent, std::string _name)
    : on(true), Draw(_parent, EntityKind::HUD, _name) {}
...
void HUD::addText(int _fontIndex, int _posX, int _posY, Color _colorText, GLfloat _scale,
                  std::string* _txt) {
...
void HUD::render(Shader* _pShader) {
    ...
    for (HUDTxt* l_pTxt : vLineText) {
        ...
        vFonts[l_pTxt->indexFonte]->RenderText(l_pTxt->pText, l_pTxt->posX, l_pTxt->posY,
                                               l_pTxt->scale);
    }
