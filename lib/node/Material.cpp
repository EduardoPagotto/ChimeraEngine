...
void Material::init() {
    ...
    for (std::map<std::string, Texture*>::iterator iTex = mapTex.begin();
...
void Material::loadTextureFromFile(const std::string& _nome, const TEX_SEQ& _seq,
...
void Material::apply(Shader* _shader) {
    ...
    for (std::map<std::string, void*>::iterator iter = mapMatVal.begin();
         iter != mapMatVal.end(); ++iter) {

        std::string nome = iter->first;
        if (nome.compare(SHADE_MAT_SHININESS) != 0)
            _shader->setGlUniform4fv(
                nome.c_str(), 1,
