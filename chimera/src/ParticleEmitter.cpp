...
ParticleEmitter::ParticleEmitter(Node* _parent, std::string _name, int _max)
    ...
    // MaxParticles = _max;
    LastUsedParticle = 0;
    material = new Material("AutoMaterialParticleEmmiter_" +
                            std::to_string(Entity::getNextSerialMaster() + 1));
}
...
void ParticleEmitter::loadTexDiffuse(const std::string& _nome,
...
int ParticleEmitter::recycleParticleLife(const glm::vec3& _camPosition) {
    ...
    for (int i = 0; i < MaxParticles; i++) {
        ...
        if (p.isDead() == false) {
            ...
            p.decrease(delta, ParticlesCount, g_particule_position_size_data,
                       g_particule_color_data, _camPosition);
            ParticlesCount++;
        }
    }
...
void ParticleEmitter::render(Shader* _pShader) {
    ...
    glBindVertexArray(
        VertexArrayID); // coloquei aqui porque acho que tenho que ligar antes
    ...
    glVertexAttribPointer(
        2, // attribute. No particular reason for 1, but must match the layout in the
           // shader.
        4, // size : r + g + b + a => 4
