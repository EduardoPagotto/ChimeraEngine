...
TileLayer::TileLayer(std::shared_ptr<ce::Shader> shader)
    ...
    shader->setUniformU("light_pos", ce::UValue(glm::vec2(4.0f, 1.5f))); //  setUniform2f("light_pos", glm::vec2(4.0f, 1.5f));
    glUseProgram(0);
}
...
void TileLayer::onRender() {
    ...
    shader->setUniformU("light_pos", ce::UValue(glm::vec2((float)(x * 32.0f / 960.0f - 16.0f), (float)(16.0f - y * 32.0f / 540.0f))));
