...
inline static TexFilter setFilter(const std::string& sParamVal) {
...
inline static TexWrap setWrap(const std::string& sParamVal) {
    if (sParamVal == "WRAP")
    ...
    bool setTextureParam(const pugi::xml_node& n, TexParam& tp) {
        for (pugi::xml_node ntPara = n.first_child(); ntPara; ntPara = ntPara.next_sibling()) {
            ...
            if (sParam == "minfilter")
                ...
                ColladaImage ci(m_colladaDom, url);
        ...
        for (pugi::xml_node prop = phong.first_child(); prop; prop = prop.next_sibling()) {
            ...
            if (p == "emission") {
                ...
                if (std::string(first.name()) == "color")
                    ...
                    ColladaImage ci(m_colladaDom, idTex);
