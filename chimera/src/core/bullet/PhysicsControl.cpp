                ...
                if (new_contacts.find(objB) == new_contacts.end()) {
                    ...
                    if (entityB) {
                        if (checkAllowCollision(entityB) == true) {
                            new_contacts[objB] =
                                std::pair<uint32_t*, uint32_t*>(static_cast<uint32_t*>(entityA), static_cast<uint32_t*>(entityB));
                        }
                        // new_contacts[objB] =
                        //     std::make_pair<uint32_t*, uint32_t*>(static_cast<uint32_t*>(entityA), static_cast<uint32_t*>(entityB));
                    }
                }
                ...
                if (new_contacts.find(objA) == new_contacts.end()) {
                    ...
                    if (entityA) {
                        if (checkAllowCollision(entityA) == true) {
                            new_contacts[objA] =
                                std::pair<uint32_t*, uint32_t*>(static_cast<uint32_t*>(entityB), static_cast<uint32_t*>(entityA));
                        }
                        // new_contacts[objA] =
                        //     std::make_pair<uint32_t*, uint32_t*>(static_cast<uint32_t*>(entityB), static_cast<uint32_t*>(entityA));
                    }
                }
            }
        }
